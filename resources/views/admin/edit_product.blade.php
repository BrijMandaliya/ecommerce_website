...
<body>
    ...
    <div class="container" style="margin-left:300px;margin-top:50px;width:75%;">
        ...
        <form method="POST" action="/updateproduct" class="updateproductform" id="updateproductform"
            ...
            <div class="collapse mb-3 catmeasurement">
                ...
                @if (count($variantData) < 1)
                    <div class="row">
                        <div class="col-3">
                            <label for="measurementName" id="catMeasurement"
                                class="form-label cat-Measurement">Enter</label>
                ...
                @else
                    @foreach ($variantData as $key => $vData)
                        <div class="row">
                            <div class="col-3">
                                <label for="measurementName" id="catMeasurement"
                                    class="form-label cat-Measurement">Enter</label>
                                <input type="text" class="form-control" id="measurementName"
                                    name="measurementValue[]" value="{{ $vData->variant_value }}" required>
                            </div>
                            <div class="col-3">
                                <label for="measurementColor" class="form-label">Enter Color</label>
                                <input type="text" class="form-control" id="measurementColor"
                                    name="measurementColor[]" value="{{ $vData->variant_colour }}">
                            </div>
                            <div class="col-2">
                                <label for="measurementStock" class="form-label">Enter Stock</label>
                                <input type="number" min="1" class="form-control" id="measurementStock"
                                    value="{{ $vData->variant_stock }}" name="measurementStock[]" required>
                            </div>
                            <div class="col-2">
                                <label for="measurementPrice" class="form-label">Enter Price</label>
                                <input type="number" class="form-control" id="measurementPrice"
                                    name="measurementPrice[]" value="{{ $vData->variant_price }}" required>
                            </div>
                            @if ($key > 0)
                                <div class="col">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="shortDescription" name="shortDescription"
                    value="{{ $data->shortDescription }}" required>
            </div>
            ...
            <div class="mb-3 images">
                ...
                <input type="file" name="p_image_1[]" id="p_image_1" multiple>
                <p class="text-danger image_error_msg" style="display: none;"></p>
                <?php
                $images = explode(',', $data->p_image_1);
                array_pop($images);
                ?>
                <div class="product_images" id="product_images" name="product_images">
                </div>

            </div>

            <div class="mt-3" style="float:right">
                <button id="product_btn" type="submit" class="btn btn-dark">Update</button>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            var brandid = {!! json_encode($data->brand_id) !!};


            getimages();

            function getimages() {
                var imageArray = {!! json_encode($images) !!};
                console.log(imageArray);

                imageArray.forEach((element, index) => {
                    var newimage = $('<img>', {
                        src: '/product_images/' + element,
                        height: "40px",
                        width: "40px",
                        id: "p_images",
                        class: index,
                        name: "p_images." + element,
                    });

                    var removeicon = $('<i>', {
                        class: "fa-solid fa-trash image-index img-index-" +
                            index,
                        id: index,
                    });
                    $("#product_images").append(newimage, removeicon);
                });


                var finalarray = "";
                var imgElements = $(".product_images").find("img");
                var numberOfImgElements = imgElements.length;

                for (var i = 0; i < numberOfImgElements; i++) {
                    $('.' + i).hover(
                        function() {
                            var deleteid = $(this).attr('class');
                            $(this).css("opacity", 0.2);
                            $('.img-index-' + deleteid).css("opacity", 1);
                        },
                        function() {
                            var deleteid = $(this).attr('class');
                            $(this).css("opacity", 1);
                            $('.img-index-' + deleteid).css("opacity", 0);
                        }
                    )
                    $('.img-index-' + i).hover(
                        function() {
                            var deleteid = $(this).attr('id');
                            $('.' + deleteid).css("opacity", 0.2);
                            $(this).css("opacity", 1);
                        },
                        function() {
                            var deleteid = $(this).attr('id');
                            $('.' + deleteid).css("opacity", 1);
                            $(this).css("opacity", 0);
                        },
                    )
                }
                ...
                for (var i = 0; i < numberOfImgElements; i++) {
                    $(document).on('click', '.' + i, function() {
                        ...
                        imageArray[id] = " ";
                        var deleteelement = document.getElementsByClassName(
                            id)[0];
                        var deletelogo = document.getElementById(id);
                        if (deleteelement) {
                            deleteelement.parentNode
                                .removeChild(
                                    deleteelement);
                            deletelogo.parentNode
                                .removeChild(
                                    deletelogo);

                        } else {
                            console.log(
                                "deleteelement or its parentNode is undefined or null"
                            );
                        }
                        finalarray = imageArray.join(',');
                        finalarray = finalarray.replace(/, /g, '', );
                        console.log(finalarray);
                        $('#product_oldimages').val(finalarray + ",");
                    });
                }
                ...
                var newinput = $('<input>', {
                    ...
                    id: 'product_id',
                    name: 'product_id',
                    value: {!! json_encode($data->id) !!},
                });

                var old_images = $("#product_oldimages")[0];
                if (old_images) {
                    old_images.parentNode.removeChild(old_images);
                }

                updateimages = $('<input>', {
                    type: 'hidden',
                    id: 'product_oldimages',
                    name: 'product_oldimages',
                    value: finalarray,
                });
                $('.updateproductform').append(newinput, updateimages);
                $('#product_oldimages').val({!! json_encode($data->p_image_1) !!});
            ...
            function getsubcategory(catid) {
                catMeasurement = $("#category").find(":selected").data('measurement');
                $("#categoryMeasurement").val(catMeasurement);
                $(".cat-Measurement").html("Enter " + catMeasurement);
                $.ajax({
                    url: '/getsubcategory',
                    type: 'POST',
                    data: 'catid=' + catid,
                    ...
                    success: function(response) {
                        response.forEach(element => {
                            var row = `<option value="` + element.id + `"  ` + (subcategoryid ==
                    ...
                    success: function(response) {
                        ...
                        response.forEach(element => {
                            if (element.sub_category_id.includes(subCat)) {
                                ...
                                var id = {!! json_encode($data->sub_category_id) !!}
                                var row = `<option value="` + element.id + `" ` + (
                                        brandid == element.id ? `selected` : ` `) +
                                    `  > ` +
